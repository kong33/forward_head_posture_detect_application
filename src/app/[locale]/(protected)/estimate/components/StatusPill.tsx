...
import { StatusPillVariant } from "@/utils/types";
...
const variantStyles: Record<StatusPillVariant, { pill: string; dot: string }> = {
  idle: {
...
export function StatusPill({ variant, children, className }: StatusPillProps) {
  ...
  return (
    ...
    >
      <span className={cn("h-1.5 w-1.5 flex-shrink-0 rounded-full", styles.dot)} aria-hidden />
      {children}
    </span>
  );
}
